...
對於處理非常大的結果集，可以利用 `cursor.itersize` 屬性來控制每次從伺服器獲取的行數。設定 `cursor.itersize = N` 會讓 Psycopg 在遍歷游標時，每次從資料庫伺服器獲取 N 行資料到客戶端記憶體中。這可以進一步減少單次記憶體載入的量，特別是在客戶端記憶體有限的情況下。合理設定 `itersize` 可以在記憶體使用和資料獲取效率之間取得平衡，避免一次性載入過多資料導致記憶體壓力，同時減少與資料庫的往返次數。
